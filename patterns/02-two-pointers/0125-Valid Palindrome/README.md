...
### 2. Array Filtering & Two Pointers Approach (Alternative)
...
* While easier to conceptualize, storing the valid characters forces us to allocate `O(N)` extra memory.
...
List comprehension and reversing both take linear time.

**Space Complexity:** `O(N)`

Space is required for the `cleaned` array and its reversed copy.