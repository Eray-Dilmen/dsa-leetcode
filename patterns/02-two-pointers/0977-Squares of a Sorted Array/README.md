...
class Solution:
    def sortedSquares(self, nums: list[int]) -> list[int]:
        # Space Complexity = O(N) -> Allocating a new array for the results
        left = 0
        right = len(nums) - 1
        result = []
        
        # Time Complexity = O(N) -> Array is traversed once (O(N)) + Reversed once (O(N))
        while left <= right:
...
A very straightforward way to solve this is to simply iterate through the array, square every element in place, and then call the built-in sort function. While this code is extremely short and easy to read, the sorting operation makes it slower.
